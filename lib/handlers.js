...
handlers._users.get = function(data, callback){
    ...
    if(phone){
        ...
        // Verify the given token is valid for the phone numbers
                                        ...
                                        _data.delete('checks', checkId, function(err){
                                            ...
                                            if(checksDeleted == checkstoDelete){
                                                if(!deletionErrors){
                                                    ...
                                                    callback(500,{'Error':'Errors encountered while attempting to delete all of the user\'s checks. All checks may note have been deleted from the system successfully'})
                                                }
                                            }
                                        });
                                    });
...
handlers._tokens.post = function(data, callback){
    ...
    const password = typeof(data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
    if(phone && password){
        // Lookup the user who match taht phone number
        _data.read('users', phone, function(err, userData){
            if(!err && userData){
                // Hash the send password and compare it to the password stored in the user object
                const hashedPassword = helpers.hash(password);
                if(hashedPassword == userData.hashedPassword){
                    // if valid, create a new token with a random name, Set expiration date 1 hour i the future
                    ...
                    // Store the token
                    _data.create('tokens', tokenId, tokentObject, function(err){
                        if(!err){
                            callback(200, tokentObject);
                        } else{
                            console.log(err);
...
// Verify if a given token id is currentyl valid for for a given user
...
// Container for all thr checks methods
handlers._checks = {};
...
handlers._checks.post = function(data, callback){
    ...
    if(protocol && url && method && successCodes && timeoutSeconds){
        // Get the token from the headders
        const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
    ...
    if(id){
        ...
        _data.read('checks', id, function(err, checkData){
            if(!err && checkData){
                // Get the token from the headers
                const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
                // Verify the given token is valid and belong to the user who created the check
            ...
            if(!err && checkData){
                ...
                handlers._tokens.verifyToken(token, checkData.userPhone, function(tokenIsValid){
                    if(tokenIsValid){
                        ...
                        _data.delete('checks', id, function(err, data){
                            if(!err){
                                // Lookup the user
                                _data.read('users', checkData.userPhone, function(err, userData){
                                    if(!err && userData){
                                        ...
                                        callback(500, {'Error' : 'Could not find the user who created the check, so could not remove the checks from the list of checks ot the user object'});
                                    }
                                });
                            } else{
                                callback(500, {'Error' : 'Could not delete the cheecks data'});        
                            }
                        });
                    } else{
                        callback(403);
